...
<div class="flex min-h-screen">
    ...
    <aside class="w-64 bg-white border-r border-slate-200 flex flex-col fixed h-screen">
        ...
        <nav class="px-3 space-y-2">
            <a href="{{ route('profile.show') }}" class="flex items-center gap-3 px-4 py-2 rounded-lg {{ request()->routeIs('profile.show') ? 'bg-[var(--primary-blue)] text-white' : 'text-slate-600 hover:bg-slate-100' }}">
     ...
     <a href="{{ route('admin.loans.index') }}"
               class="flex items-center gap-3 px-4 py-2 rounded-lg
               {{ (request()->routeIs('admin.loans.index') || request()->routeIs('admin.loans.approve') || request()->routeIs('admin.loans.reject')) ? 'bg-[var(--primary-blue)] text-white' : 'text-slate-600 hover:bg-slate-100' }}">
                <i data-lucide="test-tubes" class="w-4 h-4"></i> Pengajuan Peminjaman
            </a>

            <a href="{{ route('admin.loans.history') }}"
               class="flex items-center gap-3 px-4 py-2 rounded-lg
               {{ request()->routeIs('admin.loans.history') ? 'bg-[var(--primary-blue)] text-white' : 'text-slate-600 hover:bg-slate-100' }}">
